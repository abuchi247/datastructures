...
class DoublyLinkedList:
    ...
    def prepend(self, data):
        ...
        if self.is_empty():
            self.head = self.tail = new_node    # set the tail and head to be the new node
        else:
            new_node.next = self.head   # point the new node next pointer to the current head
            self.head.prev = new_node   # point the current head previous pointer to the new node
            self.head = new_node    # set the new node to be the new head
        self.size += 1  # increment the number of element in the linkedlist
        return

    def append(self, data):
        """
        Add element to the tail of the doubly linked list
        Time complexity: O(1)
        Space complexity: O(1)
        """
        new_node = Node(data)
        ...
        if self.is_empty():
            return self.prepend(data)  # call the prepend function
        else:
            # add new node to the tail of the linked list
            self.tail.next = new_node   # set current tail next pointer to the new node
            new_node.prev = self.tail   # set the new node prev pointer to the current tail
